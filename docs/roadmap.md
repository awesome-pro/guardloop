...
Add per-tool state machines with closed, open, and half-open states. Tool calls
that repeatedly fail will be rejected immediately during the open period.

## v0.3: Verifier Retry Loop

Add deterministic and LLM-based verifiers that can reject final outputs and
send bounded feedback into a retry attempt.
